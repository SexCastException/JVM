...
/**
 * 每个类都会使用自己的类加载器，（即加载自身的类加载器）来加载所依赖的类，
 * 如：ClassX引用了ClassY，那么ClassX的类加载就会去加载ClassY，如果ClassY还没有被加载
 *
 * 线程上下文类加载器(Context Classloader)
 * 线程上下文类加载器是从JDK 1.2开始引入的，类Thread中的getContextClassLoader()与setContextClassLoader (ClassLoader c1)分别用来获取和设置上下文类加载器。
 ...
 * 而Java核心库是由启动类加载器来加载的，而这些接口的实现却来自于不同的jar包(厂 商提供)，Java的启动类加载器是不会加载其他
